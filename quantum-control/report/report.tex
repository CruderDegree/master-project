...
%TODO More general introduction
%TODO Comparison of 2 different tools and their advanteges and disadvanteges
...
%TODO More general quantum control theory?
...
%TODO Explain parameters that Grape uses, gamma, sigma, etc
...
% TikzPicture figure
%TODO Remake to include entire workflow(?)
\begin{figure}
	\centering
	\begin{tikzpicture}[
		setup/.style={rectangle, rounded corners, draw=black, fill=black!10, inner sep=5pt},
		input/.style = {rectangle, rounded corners,text centered, draw=black, fill=green!30, node distance = 1.5cm, minimum height = 1cm, minimum width = 1cm},
		system/.style = {rectangle, rounded corners, draw=black, fill=blue!10, node distance = 1.5 cm, minimum height = 1cm, minimum width = 1cm},
		grape/.style = {rectangle, rounded corners, draw=black, fill = orange!10, inner sep = 5pt},
		grapeparam/.style = {rectangle, rounded corners, draw=black, fill=purple!40, node distance = 0.5 cm, minimum height = 1cm, minimum width = 1cm},
		simulation/.style = {rectangle, rounded corners, draw=black, fill = blue!20, inner sep = 5pt, minimum width = 2.5 cm},
		simulationparam/.style = {rectangle, rounded corners, draw=black, fill=yellow!40, node distance = 0.5 cm, minimum height = 1cm, minimum width = 1cm},
		]
		\node [setup] (Setup) {
			\begin{tikzpicture}
				\node (topLeft) [black] {};
				\node (V) [input, below of= topLeft] {$\hat{V}$};
				\node (beta) [input, below of=V] {$\beta$};
				\node (u0) [input, below of=beta] {$u_0(t)$};
				\node (T) [input, below of=u0] {$T$};				
				\node (x) [input, below of=T] {$\hat{x}$};				
				\node (H) [system, right of=V] {$\hat{H}_{GPE}$};
				\node (psi0) [system, right of= beta] {$\psi_0$};
				\node (psi0) [system, right of= u0] {$\psi_D$};
			\end{tikzpicture}
		};
		\node [black, below] at (Setup.north) {\Large Setup system};
		
		\node [black, right=of Setup] {
			\begin{tikzpicture}
				\node [grape, right=of Setup] (Grape) {
					\begin{tikzpicture}
						\node (grapetopleft) [black] {};
						\node (grapetopright) [black, right of = grapetopleft] {};
						\node [grapeparam, below=of grapetopleft] (N) {$N$};
						\node [grapeparam, below=of N] (Gamma) {$\gamma$};
						\node [grapeparam, right=of N] (Sigma) {$\sigma$};
						\node [grapeparam, right=of Gamma] (FT) {$F_{T}$};
					\end{tikzpicture}
				};
				\node [black, below] at (Grape.north) {\Large \proc{Grape}};
				
				\node [simulation, below=of Grape] (Simulation) {
					\begin{tikzpicture}
						\node (simulationtopleft) [black] {};
						\node [simulationparam, below=of simulationtopleft] (Stateplot) {$|\psi|^2(t)$};
						\node [simulationparam, below=of Stateplot] (F) {$F(t)$};
					\end{tikzpicture}
				};
				\node [black, below] at (Simulation.north) {\Large Simulation};
			\end{tikzpicture}			
		};
	\end{tikzpicture}
...
%TODO Spørg Carrie om størrelse er fin, skal der være forklarende tekst på billede etc.
\begin{figure*}
	\centering
...
%TODO Include how to do conversions between time, energy, space etc.
\subsubsection{Units}
...
%TODO Is this also the case for time-step resolution? 501 is used now, but in the QM2 paper it was 1001? Plot for 250, 500, 750, 1000, 1250? Here we would also encounter a problem when we take a too large step with numerical inaccuracy, thus small step --> better approx
...
%TODO Consider leaving out the single particle case since it is not really treated at all
% Single particle
...
%TODO Kommenter på hvordan fidelity som funktion af tid svinger så på den måde handler det om at time det rigtigt hvis man ikke vil bruge Grape
\begin{figure}
	\begin{subfigure}{0.45\columnwidth}
...
%QSL
%TODO rewrite to fit new figure
%TODO Mention how this is an estimate and that it was only optimized for 100 iterations of Grape due to the way sampling data in Composer Works, therefore, it would be a matter of trying more seeds and optimizing for longer to find a presumably even lower estimate
%TODO Also mention how the variance is pretty high for this setup and not suitable for betas far away from 0
...
%TODO Mention how the down-to-up hopes to exploit the energylandscape being more simple
...
%TODO How do I use the QEngine to plot data whatevs? 
...
%TODO Remember that the uncertainty of energies is due to the way the spectrum is calculated, and so it should technecally be recalculated each time the beta value is updated.
...
%TODO Fill me out with system specific stuff

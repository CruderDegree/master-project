...
%TODO Add [Simulation units] to beta in x-label
\begin{figure}[h]
...
%TODO Write out the actual expression used, i.e. the integral
...
%TODO I attempted to replicate these results by seeding
...
\begin{figure}
	...
	\begin{subfigure}{0.4\columnwidth}
		...
		\caption{$k=5$}
	\end{subfigure}
	\caption{Screenshots from Quantum Composer showing \proc{Grape} optimized controls for the same seeds as in Fig. \ref{fig:Clustering}. First axis is time in scaling units with a final time of $T=0.5019$. The second axis is potential displacement from $x=0$ in scaling units. Each figure shows the control seed (blue) and \proc{Grape} optimized control (red) for that seed.}
...
%TODO Fjern legend på nederste figur (repetetiv)
\begin{figure}
	\begin{subfigure}{\columnwidth}
		\centering
...
%TODO Make figure showing QM2 clustering
%TODO Make figure showing 0-crossing clustering / count 0-crossings of best solution for each T
...
%TODO Discuss why 0crossings were the most simple and objective way of classifying controls. Flips were ambigous when working with screenshots and analyzing by hand.

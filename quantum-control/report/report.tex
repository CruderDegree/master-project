...
\begin{figure}
	\centering
	\begin{tikzpicture}[
		system_setup/.style={rectangle, rounded corners, draw=black, fill=black!10, inner sep=20pt},
		inputs/.style = {rectangle, rounded corners, draw=black, fill=red!10, inner sep=10pt},
		input/.style = {rectangle, rounded corners, minimum width=1cm, minimum height=1cm, text centered, draw=black, fill=green!30, node distance = 0.5cm},
		system/.style = {rectangle, rounded corners, draw=black, fill=purple!10},
		system_comp/.style = {rectangle, rounded corners, draw=black, fill=orange!30, text centered, node distance = 0.5cm}
		]
		\node [system_setup] (SystemSetup) {
			\begin{tikzpicture}
				\node [inputs] (Inputs) {
					\begin{tikzpicture}
						\node (V) [input] {$\hat{V}$};
				...
				\node [black, below] at (Inputs.north) {Inputs};	
				
				\node [system, right=of Inputs] (System) {
					\begin{tikzpicture}
						\node (H) [system_comp] {$\hat{H}_{GPE}$};
						\node (psi0) [system_comp, below of=beta] {$\psi_0$};
					\end{tikzpicture}
				};
				
				\node [black, below] at (System.north) {System};
			\end{tikzpicture}
		};
		
		\node [black, below] at (SystemSetup.north) {System Setup};
	\end{tikzpicture}
